...
There are multiple ways to define a component in Vue 3, we want to bring up some of the most commonly seen methods in our everyday life.
...
This is the most popular one at the moment! If you've leaned React Hooks API, you may find the coding styles very similar. If you don't, don't worry! It's actually very easy to understand.
...
<!-- prettier-ignore -->
```html title="Composition API with <script setup>"
<template>
  <div>
    <div>Hello, {{ name }}, I'm {{ age }} years old.</div>
    <div>I'll be {{ ageAfter3years }} years old after 3 years.</div>
...
```html title="Composition API with setup()"
<template>
  <div>
    <div>Hello, {{ name }}, I'm {{ age }} years old.</div>
    <div>I'll be {{ ageAfter3years }} years old after 3 years.</div>
...
If you're using a SFC, we recommend you to just use `<script setup>` instead, because there are some [great features](https://vuejs.org/api/sfc-script-setup.html#typescript-only-features) `setup()` can't do!

:::caution

Since the `setup()` function is defined in an object (Options API), things like `data()`, `computed`, `methods`, `mounted()` would still work if they are defined.
...
Looks terrible, isn't it? This is bad because:
...
- `sayMyAge` is defined in `methods`, but it could also be returned in `setup()`, so now we have to check multiple places to find where a variable is being defined
