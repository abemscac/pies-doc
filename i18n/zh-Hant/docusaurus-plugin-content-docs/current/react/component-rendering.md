...
此章節對於理解響應式數值在 React 元件中的運作方式特別重要。若您在處理狀態時老是不順利，這個章節也許能拯救您。
...
用更簡單的話來說，這就像是在每次渲染都會做一次**尋找並取代**。我們來看看元件中的 `click()` 函式：
...
注意這裡所有的 `count` 都被換成了 `0`。這就是為什麼明明畫面上顯示的 `count` 是 `5`，主控台中顯示的卻是 `0`。
...
在這個範例中，當 `click()` 被執行後，`count` 的值將會是 `1` 而非 `3`。為什麼？
...
在這個範例中，`click()` 是按鈕 `onClick` 事件的處理程序，代表當按鈕被點擊時，`click()` 會是呼叫堆疊中唯一的一個函式。由於 `console.log('Done')` 是 `click()` 中的最後一個動作，`click()` 的執行會在 `console.log('Done')` 執行完成後被視為完成。因此， React 會在 `click()` 執行完成後立即依照我們所提出的更新請求 (就是 `setCount(1)`) 對狀態進行更新。
